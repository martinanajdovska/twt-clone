...
type ReplyProps = {
    self: { profilePicture?: string | null; username: string };
    tweet: ITweet;
    embedded?: boolean;
    style?: ViewStyle;
    onBeforeCompose?: () => void;
    /** If provided, overrides the default behavior (opening the compose bottom sheet). */
    onPress?: () => void;
};
...
export default function Reply({
    ...
    return (
        <TouchableOpacity
            ...
            style={[
                styles.replyBarBase,
                embedded ? null : styles.replyBarFloating,
                { backgroundColor: barBg, borderTopColor: borderColor, paddingBottom: insets.bottom + 10 },
                style,
            ]}
            onPress={() => {
                onBeforeCompose?.();
